...
```bash
cd evan/tools/linux-desktop-environment
./build-agent.sh
```
...
```python
from evan.tool_system import ToolManager
from evan.tools.bash_tool import BashToolProvider
...
```python
from evan.claude_agent import ClaudeAgent
from evan.tools import BashToolProvider
...
```python
from evan.tools.bash_tool import create_bash_tool
...
```python
from evan.tools.bash_tool_config import BashToolConfig
...
- `runtime_dir`: Base directory for agent data (default: `./evan_runtime`)
...
3. Review agent working directories:
   ```bash
   ls -la ./evan_runtime/agent-working-directory/
   ```

## License

Part of the Evan Client project.