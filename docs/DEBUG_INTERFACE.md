...
```bash
python -m evan.main debug
```

Options:
- `--port PORT`: Specify the port (default: 8069)
- `--runtime-dir DIR`: Specify runtime directory (default: evan_runtime)

Example with custom port:
```bash
python -m evan.main debug --port 8080
```
...
1. Create your tool provider in `evan/tools/`
