...
class AgentClient:
    def __init__(self, reset_state: bool = False, runtime_dir: str = None):
        print(f"{Fore.CYAN}Initializing Evan Client...{Style.RESET_ALL}")
    ...
    def start(self):
        print(f"{Fore.CYAN}Starting Evan Client...{Style.RESET_ALL}")
        self.running = True
    ...
    def status(self):
        print(f"\n{Fore.CYAN}=== Evan Client Status ==={Style.RESET_ALL}")
...
def runtime_info():
    ...
    print(f"\n{Fore.CYAN}=== Evan Runtime Information ==={Style.RESET_ALL}")
...
def shell(container_name):
    """Quick access to container shell. Usage: evan shell [container_name]"""
    from .container_shell import ContainerShell
    shell = ContainerShell()
...
def cleanup_containers(force):
    """Clean up all Docker containers created by Evan Client."""
    if not force:
