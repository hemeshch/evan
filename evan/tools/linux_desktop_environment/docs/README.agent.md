...
- **Isolated working directory**: Each agent has unique `/mnt` mounted from `evan_runtime/agent-working-directory/<ID>`
...
mkdir -p evan_runtime/agent-working-directory/my-agent
...
docker run -it --rm \
  ...
  -v $(pwd)/evan_runtime/agent-working-directory/my-agent:/mnt \
  -e AGENT_ID=my-agent \
...
# Initialize manager
manager = AgentManager(runtime_dir="./evan_runtime")
...
cat > evan_runtime/agent-working-directory/scraper/scrape.py << 'EOF'
import requests
...
mkdir -p evan_runtime/agent-working-directory/my-agent
chmod 755 evan_runtime/agent-working-directory/my-agent
...
3. **Data Management**: Regular cleanup of unused agent directories
   ...
   find evan_runtime/agent-working-directory -maxdepth 1 -type d -mtime +7 -exec rm -rf {} \;
   ```
...
4. **Monitoring**: Track resource usage
   ...
   # Check disk usage
   du -sh evan_runtime/agent-working-directory/*
   ```

5. **Security**: Never mount sensitive host directories
   ```bash
   # Good: Agent-specific directory
   -v ./evan_runtime/agent-working-directory/agent-001:/mnt
...
rm -rf evan_runtime/agent-working-directory/*
