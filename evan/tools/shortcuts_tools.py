...
class ShortcutsToolProvider(BaseToolSetProvider):
    ...
    def __init__(self, websocket_handler=None):
        ...
        # Set EVAN_SHORTCUTS_DIR to the path containing the Apple Shortcuts
        # helper scripts on your Mac. If the directory is missing, the
        # provider stays loaded but exposes no tools, so the agent can still
        # start without macOS Shortcuts configured.
        default_path = Path.home() / "evan" / "shortcut-tools"
        self.shortcuts_base = Path(os.environ.get("EVAN_SHORTCUTS_DIR", str(default_path)))
        self.available = self.shortcuts_base.exists()
        if not self.available:
            print(
                f"[shortcuts] Directory not found at {self.shortcuts_base}; "
                "Shortcuts tools disabled. Set EVAN_SHORTCUTS_DIR to enable."
            )
